...
class _RawPcapNGReader:
    ...
    def read_packet(self, size=MTU):
        while True:
            ...
            if block_type == 168627466:  # Section Header b'\x0a\x0d\x0d\x0a'
                ...
                warning("PacketNGReader: Unparsed block type %d/#%x" %
    ...
    def read_section_header(self):
        ...
        block_length, _, major_version, minor_version, section_length = struct.unpack(
            self.endian + 'IIHHi', buf)
        ...
        if options:
            ...
            for i in opt.keys():
                if not i & (0b1 << 15):
                    warning(
                        "PcapNGReader: Unparsed option %d/#%x in section header" % (i, i))
        self._check_length(block_length)

    def read_interface_description(self):
        buf = self._read_bytes(12)
        block_length, self.linktype, reserved, self.snaplen = struct.unpack(
            self.endian + 'IHHI', buf)
        ...
        if options:
            ...
            for i in opt.keys():
                if 9 in opt:
                    ...
                    warning(
                        "PcapNGReader: Unparsed option %d/#%x in enhanced packet block" % (i, i))
        try:
            self.LLcls = conf.l2types[self.linktype]
        except KeyError:
            warning(
                "RawPcapReader: unknown LL type [%i]/[%#x]. Using Raw packets" % (self.linktype, self.linktype))
    ...
    def read_enhanced_packet(self, size=MTU):
        ...
        block_length, interface, ts_high, ts_low, caplen, wirelen = struct.unpack(
            self.endian + 'IIIIII', buf)
...
def make_lined_table(*args, **kargs):
    __make_table(lambda l: "%%-%is |" % l, lambda l: "%%-%is |" % l, "",
                 seplinefunc=lambda max_length, x: "+".join(
                     ["-" * (y + 2) for y in [max_length - 1] + x + [-2]]),
                 *args, **kargs)
